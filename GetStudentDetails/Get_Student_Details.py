...
import os
# RealTimeDatabase.py
sys.path.append('/home/mubashir/Desktop/Image Processing/tkinter face recogmition GUI')
...
class GetStudentDetails:
    ...
    def getStudentDetails(self, studentDetails, message):


        # All data is taken  from GUI part 
        # year = input("Enter your year:\nFY\nSY\nTy\nBY\n").upper()
        # branch = input("\nEnter your Engineering Branch: ").upper()
        # branch = branch
        # UIN = input("\nEnter your UIN: ")
        studentDataIndatabase = self.checkStudentIsPresentInDatabase(UIN=studentDetails["UIN"])
        ...
        studentFolderPath = "GetStudentDetails/Students Encoding of Face/"+str(studentDetails["UIN"])+"_"+studentDetails["name"]
        ...
        if not studentDetails["isRegistered"]:
            if not os.path.exists(studentFolderPath):
                ...
                print("Student Folder is already exists")
        else:
    ...
    def startCamera(self, camNumber, UIN, studentFolderPath):
        ...
        while(True):
            ...
            if faceLocation:
                ...
                cv2.putText(frame, "Face is not visible", (50,70), cv2.FONT_HERSHEY_PLAIN, fontScale=3, color=(0,0,255), thickness=3)
                try:
